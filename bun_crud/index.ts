...
const server = serve({
  ...
  routes: {
    ...
    "/api/bikes": {
      GET: (req) => {
        ...
        const totalResult = dbConn.prepare(countQuery).all(...params.slice(0, -2)); // Exclude LIMIT/OFFSET params
      ...
      POST: async (req) => {
        ...
        dbConn.query(`
            ...
            VALUES (?, ?, ?, ?, ?)`).run(body.brand_id, body.name, body.year, body.base_price, body.vehicle_type)
 ...
 "/api/bikes/:id": {
      PUT: async (req) => {
        ...
        const {vehicle_type, name, base_price, year, brand_id} = await req.json()
        ...
        try {
           const stmt = dbConn.prepare(`UPDATE models SET brand_id = :brand_id, name = :name, base_price = :base_price, year = :year, vehicle_type = :vehicle_type WHERE model_id = :id`)

          const result = stmt.run(brand_id, name, base_price, year, vehicle_type, id)


          if(result.changes == 0) {
         // Verify if the ID exists
            const exists = dbConn.prepare(
            ...
            return new Response("No changes made (data identical)", { status: 200 });
          }
